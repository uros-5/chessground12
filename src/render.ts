  ...
  for (const [k, p] of plinths) {
    ...
    if (!samePieces.has(k)) {
      ...
      // assumes the new piece is not being dragged
        ...
        if (!s.plinthsPlaced) {
          const pieceName = pieceNameOf(p, s.orientation),
            pieceNode = createEl('square', pieceName) as PieceNode,
            pos = key2pos(k);
...
function isPieceNode(el: PieceNode | SquareNode): el is PieceNode {
  return el.tagName === 'PIECE';
}
function isSquareNode(el: PieceNode | SquareNode): el is SquareNode {
